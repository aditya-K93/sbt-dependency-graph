...
object DependencyGraphSettings {
  ...
  def baseSettings = Seq(
    ...
    // dependency tree
    (ignoreMissingUpdate / updateOptions) := updateOptions.value
    ...
    (ignoreMissingUpdate / ivyConfiguration) :=
      ...
      SbtAccess
        .inTask(ignoreMissingUpdate, Classpaths.mkIvyConfiguration)
        .value,
    ...
    (ignoreMissingUpdate / updateConfiguration) := updateConfiguration.value
      .withMissingOk(true),
    ignoreMissingUpdate :=
    ...
    Seq(
      ...
      moduleGraphSbt :=
        ignoreMissingUpdate.value
          ...
          .map(report ⇒
            SbtUpdateReport
              .fromConfigurationReport(report, crossProjectId.value)
          )
          .getOrElse(ModuleGraph.empty),
      moduleGraphIvyReport := IvyReport.fromReportFile(
        absoluteReportPath(ivyReport.value)
      ),
      moduleGraph := {
      ...
      moduleGraph := {
        ...
        if (filterScalaLibrary.value)
          GraphTransformations.ignoreScalaLibrary(scalaVersion, moduleGraph)
        else moduleGraph
      },
      ...
      dependencyBrowseGraphTarget := {
        target.value / "browse-dependency-graph"
      },
      ...
      dependencyDotNodeLabel := {
        (organisation: String, name: String, version: String) ⇒
      ...
      whatDependsOn := {
        val ArtifactPattern(org, name, versionFilter) =
          artifactPatternParser.parsed
        ...
        val modules =
          versionFilter match {
            ...
            case None ⇒
              graph.nodes
                .filter(m ⇒ m.id.organisation == org && m.id.name == name)
                .map(_.id)
          }
        val output =
          modules
            .map { module ⇒
              rendering.AsciiTree.asciiTree(
                GraphTransformations.reverseGraphStartingAt(graph, module)
              )
            }
            .mkString("\n")
  ...
  def ivyReportFunctionTask = Def.task {
    val ivyConfig =
      Keys.ivyConfiguration.value.asInstanceOf[InlineIvyConfiguration]
    ...
    (config: String) ⇒ {
      ...
      new File(
        ivyConfig.resolutionCacheDir.get,
        s"reports/$org-$name-$config.xml"
      )
    }
  }
  ...
  def dependencyGraphMLTask =
    Def.task {
      ...
      rendering.GraphML.saveAsGraphML(
        moduleGraph.value,
  ...
  val artifactPatternParser: Def.Initialize[State ⇒ Parser[ArtifactPattern]] =
    resolvedScoped { ctx ⇒ (state: State) ⇒
      val graph =
        loadFromContext(moduleGraphStore, ctx, state) getOrElse ModuleGraph(
          Nil,
          Nil
        )
      ...
      graph.nodes
        ...
        .map { case ((org, name), modules) ⇒
          ...
          (Space ~> token(org) ~ token(Space ~> name) ~ oneOf(versionParsers))
            .map { case ((org, name), version) ⇒
              ArtifactPattern(org, name, version)
            }
        }
        .reduceOption(_ | _)
        .getOrElse {
  ...
  def crossName(ivyModule: IvySbt#Module) =
    ivyModule.moduleSettings match {
      ...
      case hm: HasModule @unchecked
          if hm.getClass.getName == "sbt.InlineConfigurationWithExcludes" ⇒
        hm.module.name
      case _ ⇒
        throw new IllegalStateException(
          "sbt-dependency-graph plugin currently only supports InlineConfiguration of ivy settings (the default in sbt)"
        )
    }

  val VersionPattern = """(\d+)\.(\d+)\.(\d+)(?:-(.*))?""".r
  object Version {
    def unapply(str: String): Option[(Int, Int, Int, Option[String])] =
      str match {
