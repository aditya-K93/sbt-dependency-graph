...
object DOT {
  ...
  def dotGraph(
    ...
    val evictionTargetEdges =
      graph.edges
        ...
        .map { case (from, evicted) ⇒
          (
            from,
            evicted.copy(version = graph.module(evicted).evictedByVersion.get)
          )
        }
        .toSet
    ...
    val filteredEdges =
      graph.edges
        .filterNot(e ⇒
          originWasEvicted(e) || evictionTargetEdges(e)
        ) ++ evictedByEdges
  ...
  /** Render HTML labels in Angle brackets as defined at
  ...
  /** Render HTML labels with `labelType="html"` and label content in double
    * quotes as supported by dagre-d3
    */
  case object LabelTypeHtml extends HTMLLabelRendering {
    def renderLabel(labelText: String): String =
      s"""labelType="html" label="$labelText""""
  }
}
