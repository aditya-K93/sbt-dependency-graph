...
trait DependencyGraphKeys {
  ...
  val printToConsole =
    TaskKey[Unit]("printToConsole", "Prints the tasks value to the console")
  val toFile =
    InputKey[File]("toFile", "Writes the task value to the given file")
  ...
  val moduleGraph =
    TaskKey[ModuleGraph]("module-graph", "The dependency graph for a project")
  ...
  val dependencyGraph =
    InputKey[Unit]("dependency-graph", "Prints the ascii graph to the console")
  ...
  // internal
  private[graph] val ignoreMissingUpdate = TaskKey[UpdateReport](
    "dependencyUpdate",
  ...
  val whatDependsOn = InputKey[String](
    "what-depends-on",
    "Shows information about what depends on the given module"
  )
  private[graph] val crossProjectId =
    SettingKey[ModuleID]("dependency-graph-cross-project-id")
}
